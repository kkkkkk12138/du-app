...
2023 年 11 月 13 日之后创建的新个人开发者账号，在申请正式版访问前，需要按 Google 官方当前规则完成封闭测试：至少 12 名测试者连续 14 天保持加入。若控制台显示了更新要求，以当前账号页面为准。
...
7. 发布到封闭测试。
8. 把加入测试链接发给测试者。
9. 让至少 12 名测试者加入，并连续 14 天保持加入状态。
10. 提醒测试者不要中途退出；退出后重新加入不能把中断前后的天数合并为连续 14 天。
11. 收集崩溃、权限、通知、文件恢复和不同屏幕尺寸的问题。
12. 保存测试反馈、修复内容和正式发布准备说明，申请正式版访问时需要填写。

达到要求后，在 Dashboard 选择“申请正式版访问”，如实回答测试参与情况、反馈、修复和正式发布准备。Google 会审核申请；未达到人数、连续时间或参与质量要求时，可能要求继续测试。
